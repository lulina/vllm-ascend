...
If you want to run Llama or Qwen dense series models with xlite graph mode, please install xlite, and set the environment variable VLLM_ASCEND_ENABLE_XLITE.

```bash
pip install xlite
# 0: disable (default)
# 1: enable for decode only
# 2: enable for prefill and decode
export VLLM_ASCEND_ENABLE_XLITE=1
```
