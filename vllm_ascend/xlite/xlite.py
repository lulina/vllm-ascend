...
class LlamaXliteModel(XliteModel):
    ...
    def initialize(
        ...
        xlite_model.attn_norm = [
            params_dict[key] for key in params_dict
        ...
        mha_qkv_bias = [
            params_dict[key] for key in params_dict
            if "self_attn.qkv_proj.bias" in key
        ]
        q_norm = [
            params_dict[key] for key in params_dict
            if "self_attn.q_norm.weight" in key
        ]
        k_norm = [
            params_dict[key] for key in params_dict
            if "self_attn.k_norm.weight" in key
        ]
...
def xlite_model_init(
    ...
    return strategy_class().initialize(params_dict, vllm_config)
