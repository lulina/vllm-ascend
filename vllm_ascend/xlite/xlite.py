...
class Glm4MoeXliteModel(LlamaXliteModel):
    ...
    def _build_model(self, runnable: nn.Module, vllm_config: VllmConfig, config: ModelConfig) -> Model:
        ...
        xlite_model.gate_bias = [
            ...
            for layer in layers
            if hasattr(layer.mlp, "gate") and hasattr(layer.mlp.gate, "e_score_correction_bias") and layer.mlp.gate.e_score_correction_bias is not None
        ]
        xlite_model.re_up_gate = [
            layer.mlp.experts.w13_weight[i]
            for layer in layers
            if hasattr(layer.mlp, "experts") and hasattr(layer.mlp.experts, "w13_weight") and layer.mlp.experts.w13_weight is not None
            for i in range(layer.mlp.experts.local_num_experts)
        ]
        xlite_model.re_down = [
            layer.mlp.experts.w2_weight[i]
            for layer in layers
            if hasattr(layer.mlp, "experts") and hasattr(layer.mlp.experts, "w2_weight") and layer.mlp.experts.w2_weight is not None
            for i in range(layer.mlp.experts.local_num_experts)
        ]
        xlite_model.se_up_gate = [
            layer.mlp.shared_experts.gate_up_proj.weight
            for layer in layers
            if hasattr(layer.mlp, "shared_experts") and hasattr(layer.mlp.shared_experts, "gate_up_proj") and layer.mlp.shared_experts.gate_up_proj.weight is not None
        ]
        xlite_model.se_down = [
            layer.mlp.shared_experts.down_proj.weight
            for layer in layers
            if hasattr(layer.mlp, "shared_experts") and hasattr(layer.mlp.shared_experts, "down_proj") and layer.mlp.shared_experts.down_proj.weight is not None
        ]
        return xlite_model
